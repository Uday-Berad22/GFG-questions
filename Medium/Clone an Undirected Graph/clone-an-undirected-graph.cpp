...
class Solution {
public:
    void rec(Node *nn,Node *node,unordered_map<int,Node*> &m,unordered_map<int,int>  &visited){
        if(visited[node->data]>0) return;
        visited[(node->data)]++;
        // cout<<(nn->data)<<" "<<(node->data)<<endl;
          for(auto nbr: node->neighbors){
                Node *temp=m[nbr->data];
                if(temp ==NULL){
                     temp=new Node(nbr->data);
                    m[nbr->data]=temp;
                }
                (nn->neighbors).push_back(temp);
                rec(temp,nbr,m,visited);
        }
        return;
    }
    Node* cloneGraph(Node* node) {
        ...
        m[node->data]=nn;
        rec(nn,node,m,visited);
        // cout<<node->data<<endl;
        // for(auto nbr: node->neighbors){
        //     cout<<(nbr->data)<<endl;
        // }
        vector<Node*> v1=bfs(node);
        vector<Node*> v2= bfs(nn);
        // for(auto a: v1){
        //     cout<<a->data<<" ";
        // }
        // cout<<endl;
